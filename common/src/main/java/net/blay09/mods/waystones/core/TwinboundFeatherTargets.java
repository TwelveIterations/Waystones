...
public final class TwinboundFeatherTargets {
    ...
    public static void markLastSeen(ItemStack itemStack, ServerPlayer player) {
        final var featherId = itemStack.get(ModComponents.twinboundFeather.get());
        if (featherId != null) {
    ...
    public static List<Waystone> getTargets(ServerPlayer player) {
        ...
        for (final var sourceStack : findCarriedTwinboundFeathers(player)) {
            final var sourceFeatherId = sourceStack.get(ModComponents.twinboundFeather.get());
            if (sourceFeatherId == null) {
                continue;
            }

            final var sourceLinks = sourceStack.getOrDefault(ModComponents.twinboundFeatherLinks.get(), TwinboundFeatherLinksComponent.EMPTY);
            ...
            for (final var targetFeatherId : sourceLinks.links()) {
                ...
                final var targetLinks = targetStack.get().getOrDefault(ModComponents.twinboundFeatherLinks.get(), TwinboundFeatherLinksComponent.EMPTY);
    ...
    private static Optional<ItemStack> findCarriedTwinboundFeather(Player player, UUID featherId) {
        return findCarriedTwinboundFeathers(player).stream()
                .filter(it -> featherId.equals(it.get(ModComponents.twinboundFeather.get())))
                .findFirst();
    }
    ...
    private static boolean isTwinboundFeather(ItemStack itemStack) {
        return itemStack.is(ModItems.twinboundFeather);
    }
}
