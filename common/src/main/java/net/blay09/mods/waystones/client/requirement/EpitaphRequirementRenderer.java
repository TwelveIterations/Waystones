...
public class EpitaphRequirementRenderer implements RequirementRenderer<EpitaphRequirement> {
    @Override
    public void renderWidget(Player player, EpitaphRequirement requirement, GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks, int x, int y) {
        guiGraphics.renderItem(new ItemStack(ModItems.epitaph), x, y);
    }
    ...
    public void appendHoverText(Player player, EpitaphRequirement requirement, List<Component> tooltip) {
        tooltip.add(Component.translatable("gui.waystones.waystone_selection.epitaph_requirement",
                new ItemStack(ModItems.epitaph).getHoverName()).withStyle(ChatFormatting.LIGHT_PURPLE));
    }
}
